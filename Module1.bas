...
Private Function SpanHarAnnenAktivitet(ws As Worksheet, ByVal r As Long, _
    ...
    Do While c <= cMax
        ...
        ' METODE 1: Sjekk om merged cell
        If cel.MergeCells Then
            Dim ma As Range
            Set ma = cel.MergeArea

            ' Hent tekst fra merged cell
            If Len(Trim$(ma.Value)) > 0 And ma.Font.Bold Then
                txt = CStr(ma.Value)
            ...
            ' Hopp forbi hele merged area
            c = ma.Column + ma.Columns.Count
        Else
            ' METODE 2: Ikke-merged celle (gammel logikk)
...
Public Sub ApplyBlockFormatting(ws As Worksheet, målRad As Long, _
    ...
    ' OPPDATERT v2.0: Bruk merged cells-system fra Module3
    ' Kaller Module3.LagMergedAktivitet for konsistent formatering
